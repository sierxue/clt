...
Informally, the player's knowlege of $R$ is tested by picking a new point in
the plane at random from a given probability distribution $\mathcal{D}$,
and checking whether the player can decide whether the point falls inside
of or outside of $R$.
Formally, we measure the error of $R^{\prime}$ as the probability that a randomly
picked point falls in the region $R \Delta R^{\prime}$, where $R\Delta R^{\prime} = (R-R^{\prime} \cup (R^{\prime}-R)$
